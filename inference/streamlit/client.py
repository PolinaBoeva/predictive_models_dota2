...
class ModelsAPIClient:
    ...
    def get_fit_status(self, model_id: str):
        ...
        response = requests.get(
            f"{self.base_url}/fit/status", params={"model_id": model_id}
        )
        logger.info(f"Ответ от сервера: {response.status_code}, {response.json()}")
        return response.json()
    ...
    def activate_model(self, model_id: str):
        ...
        response = requests.put(
            f"{self.base_url}/activate", params={"model_id": model_id}
        )
        logger.info(f"Ответ от сервера: {response.status_code}, {response.json()}")
        return response.json()
    ...
    def predict_csv(self, csv_data):
        ...
        response = requests.post(
            f"{self.base_url}/predict_csv", files={"file": csv_data}
        )
        logger.info(f"Ответ от сервера: {response.status_code}, {response.json()}")
        return response.json()
    ...
    def get_model_info(self, model_id: str):
        ...
        response = requests.get(
            f"{self.base_url}/model_info", params={"model_id": model_id}
        )
        logger.info(f"Ответ от сервера: {response.status_code}, {response.json()}")
        return response.json()
...
class DataAPIClient:
    ...
    def get_account_ids(self):
        ...
        if response.status_code == 200:
            ...
            return (
                response.json()
            )  # Предполагается, что API возвращает список account_ids
        else:
