...
def get_catboost_params():
    ...
    hyperparameters = {}
    hyperparameters["learning_rate"] = st.number_input("Learning Rate", value=0.1, min_value=0.01, max_value=1.0)
    hyperparameters["depth"] = st.slider("Depth", min_value=1, max_value=16, value=6)
    hyperparameters["iterations"] = st.number_input("Iterations", value=100, min_value=1)
    hyperparameters["l2_leaf_reg"] = st.number_input("L2 Leaf Regularization", value=3, min_value=1, max_value=10)
    return hyperparameters
