...
def display_selected_player_info(df):
    ...
    with st.expander("### Посмотреть статистику по игроку"):
        ...
        most_matches_account_id = (
            match_count.idxmax()
        )  # account_id с наибольшим количеством матчей
        ...
        selected_metric = st.selectbox(
            "Выберите метрику для отображения", options=list(columns_to_plot.keys())
        )
...
def display_selected_player_match(df):
    ...
    with st.expander("### Посмотреть статистику по матчу"):
        ...
        radiant_win = (
            match_data["isRadiant"].iloc[0] == 1 and match_data["win"].iloc[0] == 1
        )
        ...
        players_info["Команда"] = players_info["isRadiant"].apply(
            lambda x: "Radiant" if x == 1 else "Dire"
        )
        st.write(
            players_info[["Команда", "account_id", "hero_name"]]
            .sort_values(by="Команда")
        ...
        detailed_stats = (
            df_players.groupby("account_id")[stats_to_show].mean().reset_index()
        )
        st.write(detailed_stats)

        st.write("#### Поробная статистика по игроку из матча:")
        account_id = st.selectbox(
            "Выберите account_id игрока:", players_info["account_id"]
        )
